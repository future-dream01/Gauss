...
class YOLOX(nn.Module):                 # 以nn模块中的Module类为父类创建YOLOX类
    def __init__(self, backbone=None, head=None):
        super().__init__()
        if backbone is None:
            backbone = YOLOPAFPN()      # 创建YOLOPAFPN类实例backbone
        if head is None:
            head = YOLOXHead(80)        # 创建YOLOXHead类实例head
    ...
    def forward(self, x, targets=None): # 定义前向传播函数，接受输入值x、目标值targets
        ...
        if self.training:               # 如果处于训练状态，计算损失，返回字典
            assert targets is not None  # 如果targets为空，则抛出异常
